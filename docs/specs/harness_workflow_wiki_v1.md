...
Purpose:
- Provide a minimal, deterministic gate that can be wired into CI.
- Block merges when quality floor is violated.

Command:

```bash
python scripts/eval_gate.py --run-id <run_id>
```

Default blocker policy:
- `pass_rate < 0.85`
- `fallback_rate > 0.20`
- any case status is `fail`
- schema-break-like fail reasons detected

Output contract:
- prints JSON with `ok`, `run_id`, `pass_rate`, `fallback_rate`, `blockers`
- returns exit code `0` when pass; `1` when blocked; `2` on missing/invalid summary
...
Path:
- `tests/evals/cases/<case_id>.json`

Required minimal fields:
- `id`
- `target` (`workflow|tool|wiki`)
- `input`
- `fixtures.mode` (`live|replay`)
- `expectations`

Recommended replay field:
- `fixtures.recorded_tools` (required when replay)
...
```bash
EVAL_DETERMINISTIC=1 python scripts/eval_runner.py --case wiki_concept_001 --mode replay
```

Suite:

```bash
EVAL_DETERMINISTIC=1 python scripts/eval_runner.py --suite workflow-smoke --mode replay
```
...
Cost and budget:
- `eval_results/<run_id>/cost_breakdown.json`
...
Outputs:
- `eval_results/_dashboard/dashboard.json`
- `eval_results/_dashboard/dashboard.md`

### V1 冻结标准（Day10）

Before declaring V1 freeze:
- replay mode is deterministic for target fixture cases
- schema contracts for key tools are enforced
- budget summary is present in summary and cost breakdown
- eval gate blocks obvious regressions
- docs for case authoring/replay/reading results are available
