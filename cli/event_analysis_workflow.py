...
from workflow.runner import invoke_tool_with_timing as _runner_invoke_tool_with_timing
...
LOG_PATH = os.environ.get("SONA_DEBUG_LOG_PATH", str(_PROJECT_ROOT / ".cursor" / "debug.log"))
...
def _invoke_tool_with_timing(tool_obj: Any, payload: Dict[str, Any]) -> tuple[Dict[str, Any], float]:
    ...
    tool_name = str(getattr(tool_obj, "name", "") or "")
    contract_name = tool_name if tool_name in {"extract_search_terms", "analysis_sentiment", "report_html"} else None
    return _runner_invoke_tool_with_timing(tool_obj, payload, contract_name=contract_name)
...
def run_event_analysis_workflow(
    ...
    *,
    ...
    # ============ 10.2) Wiki KB 召回快照（与 pipeline 对齐，写入过程目录供报告与答辩举证） ============
