    ...
    while True:
        try:
            ...
            if user_input.strip().startswith("/"):
                ...
                # 处理 /monitor 命令（专题监测与预警）
                if user_input.strip().startswith("/monitor"):
                    parts = user_input.strip().split()
                    monitor_config_path = None
                    monitor_task_id = None
                    if len(parts) >= 2:
                        monitor_config_path = parts[1]
                    if len(parts) >= 3:
                        monitor_task_id = parts[2]
                    run_monitor_command(monitor_config_path, monitor_task_id)
                    continue
                ...
                console.print("  [cyan]/monitor[/cyan] - 运行专题监测与预警（可选配置路径、任务ID）")
                console.print("  [dim]                 示例: /monitor 或 /monitor config/config.yaml task_demo[/dim]")
